...
# TARGET THE EXACT FAILED SCRIPTS
TARGET_IDS = ["short_cn_052", "short_cn_098", "vlog_cn_100"]
...
def run_patcher():
    print("🚑 STARTING SURGICAL LLM PATCHER FOR FAILED SCRIPTS...")
    ...
    # We only need to check the Chinese Markdown files for these 3 targets
    targets = [
    ...
    for t in targets:
        ...
        for idx, content in enumerate(raw_scripts):
            ...
            # Skip everything unless it's one of our 3 failures
            ...
            prompt = (
                ...
                f"  ]\n"
                f"}}\n"
                f"OUTPUT ONLY VALID JSON. IT MUST CONTAIN SCENES."
            )

            # Retry logic: Give it 3 attempts to return a valid, non-empty array
            attempt = 0
            success = False
            while attempt < 3 and not success:
                attempt += 1
                try:
                    response = client.chat.completions.create(model=MODEL_ID, messages=[{"role": "user", "content": prompt}], temperature=0.1)
                    ...
                    # THE GUARDRAIL: Reject empty arrays
                    if not data["scenes"]:
                        raise ValueError("LLM generated an empty array. Retrying...")

                    with open(out_file, 'w', encoding='utf-8') as f:
                        json.dump(data, f, indent=2, ensure_ascii=False)
                    print(f"✅ {sid} | FIXED & SAVED! (Attempt {attempt})")
                    success = True
                    
                except Exception as e:
                    print(f"⚠️ {sid} | Attempt {attempt} Failed: {str(e)}")
            
            if not success:
                print(f"❌ {sid} | FAILED AFTER 3 ATTEMPTS. Total hallucination.")
